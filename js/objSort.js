...
objSort={
  init:function(){
    Array.prototype.objSort=function(){
      ...
      return this.sort(objSort.sorter);
    };
    Array.prototype.strSort=function(){
      objSort.setThings(this);
      return this.sort(objSort.charSorter)
    }
  },
  sorter:function(x,y){
    var a=objSort.a
    ...
    for(var i=0;i<a.length;i++){
      ...
      r=m-n;
      if(isNaN(r)){r=objSort.charSorter(m,n)};
      if(r!=0){return r*d[i]}
    }
    return r
  },
  charSorter:function(x,y){
    if(objSort.ignoreCase){x=x.toLowerCase();y=y.toLowerCase()};
    var s=objSort.chars;
